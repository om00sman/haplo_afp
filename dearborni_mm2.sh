...
mm2_m=$(samtools view "$out/${name}_mm2_alignment.sorted.bam" | awk '{print $1}' | sort | uniq | grep -F -f "$out/${name}_m.txt" | wc -l)
mm2_a=$(samtools view "$out/${name}_mm2_alignment.sorted.bam" | awk '{print $1}' | sort | uniq | grep -F -f "$out/${name}_a.txt" | wc -l)
mm2_p=$(samtools view "$out/${name}_mm2_alignment.sorted.bam" | awk '{print $1}' | sort | uniq | grep -F -f "$out/${name}_p.txt" | wc -l)
