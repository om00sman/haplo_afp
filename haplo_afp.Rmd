...
tree_panel <- ggtree(hap_afp_tree) + 
  geom_tiplab(fontface = "italic", parse = TRUE, size = 6.5) + 
  theme_tree2() +
  geom_tree(size = 1.5) +
  labs(x = "Mya") +
  theme(
  ...
  scale_x_continuous(
    limits = c(-2, tree_age + 25),  # Extends left for labels, ends at 0
...
haplo_afp <- read_excel("C:/Users/omoos/Downloads/zoarcoidei_haplo-afp (16).xlsx", sheet = "fig_6")
...
dotplot <- ggplot(haplo_afp, aes(
  ...
  labs(y = "", x = "AFP Count") +
  scale_color_manual(values = c("hap1" = "#D71920", "hap2" = "#5BCBF5")) +
  ...
  theme(strip.text = element_blank(),  # Remove facet labels
        ...
        axis.title.x = element_text(size = 20),# change size of x axis text
        axis.text.x = element_text(size = 15),
...
fig6 <- cowplot::plot_grid(tree_panel, dotplot, rel_widths = c(1,1.3))
